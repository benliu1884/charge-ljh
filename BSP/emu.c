...
/*1) 计算角差校正值
		...
		if(err1>=0){emu_ecr_reg.ECR.GPhs1 =(uint32_t)(err1*0x8000);}
		else {emu_ecr_reg.ECR.GPhs1 =(uint32_t)(0x10000+err1*0x8000);}  
...
/*	2) 计算校正角差后的功率值 P’
		...
		if (err1>=0){emu_ecr_reg.ECR.GP1 =(uint32_t)(err1*0x8000);}
		else {emu_ecr_reg.ECR.GP1 =(uint32_t)(0x10000+err1*0x8000);}  
